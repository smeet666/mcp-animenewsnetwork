...
`mcp-animenewsnetwork` is a read-only client for [Anime News Network](https://www.animenewsnetwork.com). It runs on your
...
| -------------------------- | -------------------------- |
| `www.animenewsnetwork.com` | the encyclopedia's pages   |
| `cdn.animenewsnetwork.com` | the encyclopedia's own API |
...
| A `User-Agent`         | `mcp-animenewsnetwork/<version> (+https://github.com/smeet666/mcp-animenewsnetwork)`, so the site can reach a person about the traffic it receives. |
| Your IP address        | Sent by your network to any host you contact, as with any web request.                                                                              |

Your requests reach Anime News Network. What is done with them there is governed by that site's own privacy policy, which this project does not control.
...
becomes of them. `ANN_LOG_LEVEL` governs how much is written and defaults to `error`. These lines stay on your machine.

## The settings that change any of this

| Variable           | What it changes                                                                                                  |
| ------------------ | ---------------------------------------------------------------------------------------------------------------- |
| `ANN_USER_AGENT`   | Adds your own identifier in front of this project's, which stays appended so the site can always reach a person. |
| `ANN_CACHE_TTL_MS` | How long an answer is held in memory. `0` turns the cache off.                                                   |
| `ANN_LOG_LEVEL`    | How much is written to stderr.                                                                                   |
...
Open an issue on [the repository](https://github.com/smeet666/mcp-animenewsnetwork/issues). For something exploitable,
...
`mcp-animenewsnetwork` est un client en lecture seule pour [Anime News Network](https://www.animenewsnetwork.com). Il
...
| -------------------------- | --------------------------- |
| `www.animenewsnetwork.com` | les pages de l'encyclopédie |
| `cdn.animenewsnetwork.com` | l'API de l'encyclopédie     |
...
| Un `User-Agent`   | `mcp-animenewsnetwork/<version> (+https://github.com/smeet666/mcp-animenewsnetwork)`, pour que le site puisse joindre une personne au sujet du trafic qu'il reçoit. |
| Votre adresse IP  | Transmise par votre réseau à tout hôte que vous joignez, comme pour n'importe quelle requête web.                                                                   |

Vos requêtes atteignent Anime News Network. Ce qui en est fait là-bas relève de la politique de confidentialité de ce site, que ce projet ne contrôle pas.
...
qu'ils deviennent. `ANN_LOG_LEVEL` règle leur quantité et vaut `error` par défaut. Ces lignes restent sur votre machine.

## Les réglages qui changent tout cela

| Variable           | Ce qu'elle change                                                                                                        |
| ------------------ | ------------------------------------------------------------------------------------------------------------------------ |
| `ANN_USER_AGENT`   | Ajoute votre identifiant devant celui du projet, qui reste accolé pour que le site puisse toujours joindre une personne. |
| `ANN_CACHE_TTL_MS` | Combien de temps une réponse est gardée en mémoire. `0` éteint le cache.                                                 |
| `ANN_LOG_LEVEL`    | La quantité écrite sur stderr.                                                                                           |
...
Ouvrez une issue sur [le dépôt](https://github.com/smeet666/mcp-animenewsnetwork/issues). Pour quelque chose
