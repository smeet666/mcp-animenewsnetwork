...
Open an issue: <https://github.com/smeet666/mcp-animenewsnetwork/issues>
...
an entry whose cast comes back empty although the page lists one is a real bug, and it is only visible to
...
It reads Anime News Network and returns what it reads. It does not write anything back, it
...
- **Every result carries its source.** Content that came from Anime News Network goes out
  with a link to the page it came from, and the text block keeps that link even
  when it has to be shortened.
- **The server paces itself.** Anime News Network is someone else's site. The minimum
...
`ANN_LIVE=1 npm run test:live`, which does: it makes one request per route
against https://www.animenewsnetwork.com and checks the fields the parsers depend on. Run it if you
...
The tools are `search_titles`, `get_title`, `list_recent` and `get_news`. A new tool is a bigger conversation than a new field on
...
## When Anime News Network changes
